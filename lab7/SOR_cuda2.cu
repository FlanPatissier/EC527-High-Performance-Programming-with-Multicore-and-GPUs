...
__global__ void SOR_GPU(float *arr, int OMEGA, int x_len, int y_len)
{
    ...
    int local_tid_x = threadIdx.x; // thread_id_x
    int local_tid_y = threadIdx.y; // thread_id_y

    int block_row = blockDim.x; // 16
    int block_col = blockDim.y; // 16

	int block_idx = blockIdx.x; // block_id_x
	int block_idy = blockIdx.y; // block_id_y

	int i = block_idx * block_row + local_tid_x;
	int j = block_idy * block_col + local_tid_y;
