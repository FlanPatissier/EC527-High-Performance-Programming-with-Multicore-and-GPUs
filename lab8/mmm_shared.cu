...
nvcc  mmm_shared.cu -o mmm_shared
...
__global__ void kernel_mmm(float *arr1, float *arr2, float *arr3, int arrlen)
{
    ...
    for (int m = 0; m < arrlen / TILE_WIDTH; ++m)
    {
        ...
        // in each m loop the Nds and the Mds calculate each block's whole values by adding into the Pvalue
        // And the threads all work at the same time
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < MATRIX_SIZE; i++)
    {
        for (j = 0; j < MATRIX_SIZE; j++)
        {
            ...
            if(h_result_CPU[index] >= h_result_GPU[index]*1.01 or  h_result_CPU[index] <= h_result_GPU[index]*0.99){
                errors++;
            }
        }
    }
